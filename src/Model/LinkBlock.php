 ...
 * Class LinkBlock.
 */
class Link_Block extends \Object_Model
{
    /**
     * @var int
     */
    ...
    {
        Shop::add_table_association('link_block', ['type' => 'shop']);
        parent::__construct($id, $id_lang, $id_shop);
        if ($this->id) {
            $this->content = json_decode($this->content, true);
            if ($this->custom_content) {
                $this->custom_content = array_map(fn($el) => json_decode($el ?? '', true), $this->custom_content);
            }
        }
        if (is_null($this->content)) {
            $this->content = ['cms' => [], 'product' => [], 'static' => [], 'category' => []];
        }
    }
    ...
    {
        return ['id' => $this->id, 'id_link_block' => $this->id, 'name' => $this->name, 'id_hook' => $this->id_hook, 'position' => $this->position, 'content' => $this->content, 'custom_content' => $this->custom_content, 'shop_association' => $this->get_associated_shops()];
    }
}